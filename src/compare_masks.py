...
Loads each matched pair with PyAIMS and computes one or both of:

  wasserstein  – approximate 3-D Earth Mover's Distance (in voxels) via
                 axis-marginal projections, grouped into bucket steps.
...
Select the metric with --metric wasserstein|diff|both.
...
def main(argv=None):
    ...
    use_wass = args.metric in ("wasserstein", "both")
    use_diff = args.metric in ("diff", "both")
    ...
    if use_wass and wass_buckets:
        ...
        for label, names in sort_buckets(wass_buckets).items():
            print(f"  {label:10s}  {len(names):3d} mask(s)")

    if use_diff and diff_buckets:
        print(f"\nDiff buckets (step={step}):")
        for label, names in sort_buckets(diff_buckets).items():
            print(f"  {label:10s}  {len(names):3d} mask(s)")
