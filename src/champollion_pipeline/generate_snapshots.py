...
def generate_sulcal_graph_snapshot(
    ...
    Args:
        ...
        a: Existing Anatomist instance to reuse. If None a new one is created.
    """
    ...
    a.removeObjects(objects, [win])
    a.closeWindows([win])
    a.deleteObjects(objects)
...
def generate_tiles_snapshot(crops_dir, output_path, size=(800, 600), level=1, champollion_data_root=None):
    ...
    for hemi_name, side, quat, mesh_file in hemispheres:
        ...
        a.removeObjects([reg_graph, mesh], [win])
        a.closeWindows([win])
        a.deleteObjects([reg_graph, mesh])

        group += 1

    a.deleteObjects([anom])
    return snapshots
